...
(async () => {
  try {
    const [
      ...
      fs.readFile(
        path.join(__dirname, 'templates/react-native/Icon.d.ts.hbs'),
        'utf-8'
      ),
      fs.readFile(
        path.join(__dirname, 'templates/react-native/Icon.js.hbs'),
        'utf-8'
      ),
      fs.readFile(
        path.join(__dirname, 'templates/react-native/index.js.hbs'),
        'utf-8'
      ),
      fs.readFile(
        path.join(__dirname, 'templates/react-native/styles.js.hbs'),
        'utf-8'
      ),
      fs.mkdir(path.join(__dirname, '../react-native/src'), {
        recursive: true,
      }),
    ]);
    ...
    const iconTypescriptTemplate = Handlebars.compile(
      iconTypeScriptTemplateContent,
      { noEscape: true }
    );
    ...
    await fs.mkdir(
      path.join(
        __dirname,
        '../demos/react-native/node_modules/multiplatformicon-react-native'
      ),
      { recursive: true }
    );
    ...
    await Promise.all([
      promisify(exec)(
        `cp -r ${path.join(__dirname, '../react-native/')}* ${path.join(
          __dirname,
          '../demos/react-native/node_modules/multiplatformicon-react-native/'
        )}`
      ),
